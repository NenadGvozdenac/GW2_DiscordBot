...
public class SlashCommandInteraction extends ListenerAdapter {
    ...
    private void PING_COMMAND(@NotNull SlashCommandInteractionEvent event) {
        ...
        event.getHook().sendMessage("`BEEP BOOP, CALCULATING PING...`").queue(k -> {
            ...
            eb.addField("\uD83E\uDD16 BOT PING \uD83E\uDD16", String.valueOf(time) + " ms.", true);
            eb.addField("\uD83E\uDD16 WEBSOCKET PING \uD83E\uDD16", String.valueOf(event.getJDA().getGatewayPing()) + " ms.", true);

            k.editMessage("`BEEP BOOP, PING CALCULATED...`").setEmbeds(eb.build()).queueAfter(2, TimeUnit.SECONDS);
        });
    }
}
