...
public class DailyAchievements {
    ...
    public void ReadFractalsFromApi() {
        new Timer().schedule(new TimerTask() {
            ...
            public void run() {
                ...
                Boolean isFractalAlreadySent = getIsFractalAlreadySent(Constants.dailyAchievementsChannelID);
                if(!isFractalAlreadySent) {
    ...
    protected Boolean getIsFractalAlreadySent(String dailyAchievementsChannelID) {
        Message latestMessage = Main.jda.getTextChannelById(dailyAchievementsChannelID).getHistory().retrievePast(1).complete().get(0);
